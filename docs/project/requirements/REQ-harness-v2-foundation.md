...
v2 では `owox` の利用を必須にします。個別 CLI の rules file や skill ディレクトリだけで最低限回る互換モードは必須要件にしません。

AI は task 進行、verify、gate、handoff、完了判定を `owox` 経由で行う必要があります。
...
前提資料、required decisions、required checks、required evidence が不足している場合は deterministic に停止できる必要があります。
...
`.owox/`、CLI 向け rules file、CLI 向け adapter files、CLI 固有の生成ディレクトリ、task 雛形、docs 雛形は、初期状態の手書き正本ではなく、`harness-init` によって機械生成または管理される必要があります。

AI 専用 artifact は `.owox/` に集約し、人間向け正本は `docs/project/` に残す必要があります。
...
- CLI 向け rules file と CLI 固有の生成ディレクトリを生成する
- `.owox/` の管理対象を生成または初期化する
- docs 雛形を生成する
...
handoff は低コンテキスト運用を前提とした圧縮 packet を扱える必要があります。
...
`owox` を経由しない進行、前提不足、未解決 gate、evidence 不足も deterministic に扱える必要があります。
...
CLI 固有の生成ディレクトリと `.owox/` の責務差分も adapter 境界を越えて core に漏らさない必要があります。
...
- `../architecture.md`
- `REQ-intent-governed-agent-control.md`
