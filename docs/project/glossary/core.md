...
source of truth から生成または管理される project 向けの配布物です。`.owox/`、各 CLI 向け rules file、各 CLI 向け生成ディレクトリ、docs skeleton、adapter files などを含みます。
...
ユーザーが最終的に実現したい状態、壊したくない前提、優先順位、未解決事項を保持する上位契約です。task は intent に従属します。

### decision ledger

人間または親エージェントが確定した判断を構造化して保持する記録です。会話履歴の代わりに再利用します。

### context packet

親エージェントから子エージェントへ渡す圧縮 handoff です。intent summary、task contract、related decisions などを含みます。

### prerequisite enforcement

必要な資料、判断、検証、evidence がそろっていない場合に task を先へ進めない決定的制御です。

### `.owox/`

`owox` が扱う AI 専用 artifact の保存先です。hidden context、runtime state、intent、decision ledger、context packet、evidence index などを置きます。
