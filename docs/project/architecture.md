...
`owox-harness` v2 は、generator-driven / workflow-driven / subagent-native なメタハーネスです。主役は `owox` の source layer と runtime tools であり、`.agents/` や `AGENTS.md` は生成物です。

## 不変条件

### INV-1. source of truth は生成物ではない

次は source of truth ではありません。
...
CLI ごとの hooks、skills、custom agents、MCP、config 差分は adapter に集約し、core workflow に散らさないことを原則とします。
...
1. `owox.harness.yaml` を source of truth として読む
