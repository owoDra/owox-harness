...
- `.agents/patterns/index.md` があれば参照して、再利用すべき既存 pattern の有無を確認する
...
- `.agents/requirements/index.md` を参照して関連 requirement の位置づけと重複有無を確認する
- `.agents/specs/index.md` のルールを参照して、`project.kind` に応じた spec の配置先を確認する
...
1. `request_user_input` で、対象 requirement、扱うカテゴリ、求める振る舞い、入出力、状態、エラー、横断ルール、互換性条件の有無を確認する
2. `project.kind` に応じて spec の配置先を決める
   - `simple`: `.agents/specs/` 直下に配置する
   - `monorepo` かつプロジェクト共通: `.agents/specs/shared/` に配置する
   - `monorepo` かつ特定 subproject 向け: `.agents/specs/<subproject>/` に配置する
3. 既存の `.agents/specs/` と `.agents/specs/index.md` を確認し、重複 spec や近い振る舞い定義を調べる
4. 関連する `.agents/requirements/`、`.agents/adr/`、`.agents/validation.md`、実装コード、テストコードを確認し、既存 spec と矛盾する振る舞いや前提変更がないかを調べる
5. 重複・矛盾確認の結果を整理する
...
8. フォーマットに従って spec を追加または更新する
   ...
   - 必要なディレクトリがなければ作成する
9. `.agents/specs/index.md` に索引を追加または更新する
...
- spec の配置先は `project.kind` に従う
  - `simple` は `.agents/specs/` 直下
  - `monorepo` の共通 spec は `.agents/specs/shared/`
  - `monorepo` の subproject 固有 spec は `.agents/specs/<subproject>/`
...
- `request_user_input` または同等の確認で対象 requirement、カテゴリ、主要な入出力と振る舞いを確認した
- 既存 spec との重複を確認した
