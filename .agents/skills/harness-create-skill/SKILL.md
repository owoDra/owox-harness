...
- `./references/SKILL.example.md` を参照して `.agents/skills/<skill-name>/SKILL.md` のフォーマットを把握
...
- `.agents/glossary.md` を参照して既存の用語と命名を把握する

## 前提知識

- 既存のスキル（`.agents/skills/`）— 構成の参考にする
...
| プレフィックス | 用途 |
| ------------ | -------------------------------------- |
| `docs-`      | ハーネス資料の生成・変換・更新を行うスキル |
| `harness-`   | ハーネスライフサイクルの関連のスキル |
...
2. `.agents/skills/<skill-name>/SKILL.md` を作成する。
