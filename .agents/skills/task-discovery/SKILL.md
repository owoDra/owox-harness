...
1. ユーザー依頼と `task-prepare` の確認結果をもとに、`request_user_input` ツールで `goal`、`execution_mode`、禁止事項、完了条件を確認する
...
7. 調査結果を以下の観点で整理する
   ...
   - 次に必要な意思決定
8. 判断ゲートで停止して確認する
   - `agent-led`: 次の作業に進む前に、重要な前提変更、権限外変更、高リスク判断が出た場合のみ `request_user_input` ツールで確認する
   - `collab-led`: 調査方針確定後、主要論点整理後、次アクション提案前に `request_user_input` ツールで確認する
9. 後続に渡せる成果物を残す
   - 調査サマリ
   - 影響範囲一覧
   - 未解決事項一覧
...
- ユーザー判断が必要な確認は口頭確認だけで済ませず、必ず `request_user_input` ツールを使う
...
- ユーザー判断が必要な項目で `request_user_input` ツールを使っている
- 事実、推測、未解決事項が分離されている
