...
このハーネスでは、ハーネスの正本を `.agents/` に、プロジェクト資料の正本を `docs/project/` に分離します。
...
`.agents/` はハーネス本体です。エージェントの制約、作業手順、skills、tasks、scripts をここに集約します。

- `.agents/project.yaml`
  ハーネスが参照するプロジェクトの最小情報を保持する
- `.agents/skills/`
  作業種別ごとの進め方を定義する
- `.agents/tasks/`
  進行中の作業と完了条件を記録する
