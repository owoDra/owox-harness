...
1. ユーザー依頼と `task-prepare` の確認結果をもとに、`question` ツールで `goal`、`execution_mode`、変更可能範囲、禁止事項、完了条件を確認する
...
11. 判断ゲートで停止して確認する
   - `agent-led`: 重要な前提変更、権限外変更、高リスク変更、仕様変更が必要になった場合のみ `question` ツールで確認する
   - `collab-led`: 実装方針確定後、主要変更前、検証結果整理後に `question` ツールで確認する
