...
1. ユーザー依頼と `task-prepare` の確認結果をもとに、`question` ツールで `goal`、`execution_mode`、レビュー対象、禁止事項、完了条件を確認する
...
8. 判断ゲートで停止して確認する
   - `agent-led`: 指摘採否に大きく影響する前提差異、高リスク懸念、レビュー対象の認識違いがある場合のみ `question` ツールで確認する
   - `collab-led`: 優先観点確定後、主要 finding 整理後、次アクション提案前に `question` ツールで確認する
9. 後続に渡せる成果物を残す
   - 指摘事項一覧
   - 未解決事項
   - 推奨する次タスク種別
