...
- 既に存在する場合は `question` ツールで現在のハーネス設定をすべて削除して初期化しなおすかを確認する
- `question` ツールが使えない場合は通常メッセージで同じ質問を順番に行い、回答を task に転記する
...
- 通常メッセージで確認した場合も、`question` を使った場合と同じ粒度で回答を task に残す
