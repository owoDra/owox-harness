...
- `docs/project/index.md`、`docs/project/adr/index.md`、関連資料を参照して、対象プロジェクトの文脈、利用言語、構成、責務チーム、外部依存、影響範囲を把握する
- `docs/project/glossary.md` を参照して用語、命名、判断対象の表記を統一する
- `./references/adr.template.md` を参照して ADR の基本フォーマットを把握する
- `./references/best-practices.md` を参照して ADR に残すべき判断と書き方を把握する
- `docs/project/adr/index.md` を参照して既存の採番規則と索引形式を把握する
...
1. `question` で、判断内容、背景、候補案、決定状態、関連 requirement / spec を確認する
2. 既存の `docs/project/adr/` と `docs/project/adr/index.md` を調べ、同じ判断が既に記録されていないか確認する
3. ADR にする価値があるかを判断する
   - 設計・実装・運用に継続的な影響がある
   - 代替案やトレードオフがある
   - 将来「なぜそうしたか」を説明する必要がある
4. 既存 ADR の改訂である場合は、どの ADR を supersede / deprecate するか決める
5. 現在のプロジェクト資料構成を確認し、配置先を決める
   - 単一構成: `docs/project/adr/`
   - 複数サブプロジェクト構成: `docs/project/adr/<subproject>/`
   - 複数サブプロジェクト構成では、対象サブプロジェクト配下へ置くか、共通判断として `docs/project/adr/` 直下に置くかを確認する
...
8. `docs/project/adr/index.md` に索引を追加または更新する
...
- `question` または同等の確認で判断の対象と背景を確認した
- 既存 ADR との重複や改訂関係を確認した
- 単一構成 / 複数サブプロジェクト構成に応じた配置先を確認した
- 採番と index 更新が整合している
